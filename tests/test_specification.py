    ...
    def test_evaluator_identity_is_bound_for_every_execution(self):
        ...
        for scope, text, phrases in (
            ("manifest", manifest, (
                ...
                "unsigned canonical payload includes `producer_id` and `producer_provenance`",
                "unsigned canonical payload that omits both `evaluator_identity_evidence_ref` and `evaluator_identity_evidence_sha256`",
                ...
                "binding is retained outside worker/candidate authority and does not contain this record's self-derived reference or digest",
            ...
            ("preflight", preflight, (
                ...
                "record's `producer_id` and `producer_provenance` are retained from the authenticated trusted recorder",
                ...
                "validate `producer_id` and `producer_provenance` against the retained trusted assignment/registration binding for that exact evidence payload",
            ...
            ("scoring", scoring, (
                ...
                "Resolve the evidence reference and verify its digest, the payload's `producer_id` and `producer_provenance` against the retained trusted assignment/registration binding for that exact evidence payload",
                "Missing, unknown, unauthenticated, or mismatched producer provenance is `EVIDENCE_INCOMPLETE`",
            ...
            ("retained", retained, (
                "their effective evaluator adapter/source/runtime identities, the `EVALUATOR_IDENTITY_VERIFIED` records, their `producer_id` and `producer_provenance` values, the retained trusted assignment/registration bindings required to authenticate each evidence payload, their content-addressed `evaluator_identity_evidence_ref` values and `evaluator_identity_evidence_sha256` digests, and bound verification evidence",
