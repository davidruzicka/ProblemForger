...
- cost, latency, token, graph-overhead, invalidation, and propagation metrics are retained; provider-cost and elapsed-time-per-resolution use frozen configuration-level numerators/denominators, include infrastructure-invalid/replacement attempts, and report NA rather than inventing a value when there are zero resolved slots;
...
- every whole-run attempt records absolute UTC attempt/semantic-start/end timestamps plus provider/API/model revision or deployment metadata when exposed, enabling post-hoc provider-drift analysis;
